...
\end{equation}
where $\text{PE}(i)$ is the fixed sinusoidal positional encoding
...
\begin{equation}
  ...
  \label{eq:lr_proj}
\end{equation}
This projector is \emph{discarded} before Stage~3.
...
\begin{align}
  ...
  \label{eq:ner_logits}
\end{align}
where $\bH = \{\bh^{(4)}_i\}_{i=1}^{m}$ is the backbone output for
\emph{noisy} input words.
...
\label{eq:label_proj}
\end{equation}
where the continuation function $\sigma : \calY \to \calY$ maps
\texttt{B-X} $\mapsto$ \texttt{I-X} and is the identity on all other labels.
This ensures BIO validity is preserved after word splitting.
...
\begin{align}
  ...
  \label{eq:loss_class}
\end{align}
where $\hat{p}_{j,c} = \text{softmax}(\text{logits}_j)_c$.
