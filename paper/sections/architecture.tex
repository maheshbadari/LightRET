...
\begin{equation}
  ...
  \label{eq:retvec}
\end{equation}
where $\phi(w) \in \RR^{d_{\text{in}}}$ is a fixed character-hash projection.
All weights $\{\bW_1, \bW_2, \bW_3, \bb_1, \bb_2, \bb_3\}$ are pretrained
and \textbf{frozen throughout all three stages}.
The output lies in $[-1, 1]^{256}$ (bounded by $\tanh$).
...
\begin{align}
  ...
  \label{eq:pre_ln_ffn}
\end{align}
where $\text{MHA}$ denotes multi-head self-attention and
$\text{FFN}(\bv) = \bW_2\,\text{GELU}(\bW_1\bv + \bb_1) + \bb_2$.
